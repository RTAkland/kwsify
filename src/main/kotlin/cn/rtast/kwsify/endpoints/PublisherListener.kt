...
import cn.rtast.kwsify.sentSuccessfully
    ...
    fun onEvent(connection: WebSocket, clientId: String, channel: String, payload: String) {
        KwServer.sessions.forEach {
            if (it.clientId == clientId && it.channel == channel) {
                it.session.send(Reply(getTimestamp(), MsgType.Message, payload).toJsonString())
                connection.send(Reply(getTimestamp(), MsgType.Notify, sentSuccessfully).toJsonString())
            }
        }
    }
}